...
The selected result is centred where the compacted group bounds permit; at an outer edge, the group is clamped so the viewport does not expose empty space. Arrow-key navigation smoothly retargets the filtered presentation with Hyprland's `windowsMove` animation while the real scrolling layout continues to provide normal focus synchronization. Rapid repeated navigation continues from the currently displayed intermediate position.
